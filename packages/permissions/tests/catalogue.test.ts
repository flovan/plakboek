...
describe('PERMISSION_GROUPS', () => {
  ...
  it("has at least one permission per group, and every permission's group is listed", () => {
    const groupsInUse = new Set(
      Object.values(catalogue.PERMISSIONS).map((meta) => meta.group),
    );
...
describe('edit/publish separation (D-06)', () => {
  it.each(['pages', 'entries', 'templates'] as const)(
    ...
    (resource) => {
      expect(Object.hasOwn(catalogue.PERMISSIONS, `${resource}:edit`)).toBe(
        true,
      );
      expect(Object.hasOwn(catalogue.PERMISSIONS, `${resource}:publish`)).toBe(
        true,
      );
    },
  );
...
describe('descriptor metadata (D-08)', () => {
  it('every label/description id follows the permissions.<resource>.<action>.<kind> shape, unique, non-empty, single-line', () => {
    ...
    for (const [, meta] of entries) {
      ...
      expect(meta.description.id).toBe(
        `${meta.label.id.slice(0, -'.label'.length)}.description`,
      );
...
describe('catalogue immutability', () => {
  it('throws when assigning a new key on PERMISSIONS', () => {
    expect(() => {
      ...
      catalogue.PERMISSIONS['pages:own-edit'] =
        catalogue.PERMISSIONS['pages:edit'];
    }).toThrow(TypeError);
  });
