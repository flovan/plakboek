...
function assertPostgresConnectionString(connectionString: string): void {
	const isNonEmptyString = typeof connectionString === "string" && connectionString.length > 0;
	const hasPostgresScheme =
		isNonEmptyString && POSTGRES_SCHEMES.some((scheme) => connectionString.startsWith(scheme));
