...
import { MigrationFailedError, migrateWithRegistry } from "../../src/migrate.js";
import {
...
async function tableExists(connectionString: string, relationName: string): Promise<boolean> {
...
async function createGateTable(connectionString: string): Promise<void> {
	...
	await client.connect();
	try {
		await client.query("CREATE TABLE partial_failure_gate (id serial primary key)");
	} finally {
		await client.end();
	}
}
...
describe("re-running after a partial non-transactional failure completes cleanly", () => {
	it("run 1 fails at statement index 2, leaving partial_a and its index but not partial_b, and no bookkeeping row", async () => {
		...
		let caught: unknown;
		try {
			await migrateWithRegistry({ connectionString: testDb.connectionString, migrations });
		} catch (error) {
			caught = error;
		}
		...
		expect(await tableExists(testDb.connectionString, "partial_a_id_idx")).toBe(true);
	...
	it("run 2 after the gate exists completes the migration; run 3 applies nothing", async () => {
		...
		await migrateWithRegistry({ connectionString: testDb.connectionString, migrations }).catch(
			() => undefined,
		);
...
describe("re-running after a partial transactional failure completes cleanly", () => {
	it("run 1 rejects and rolls back every statement, including the first", async () => {
		...
		expect(await tableExists(testDb.connectionString, "partial_tx_a")).toBe(false);
		expect(await bookkeepingRowCount(testDb.connectionString)).toBe(0);
	});
	...
	it("run 2 after the gate exists applies the migration", async () => {
		...
		expect(await tableExists(testDb.connectionString, "partial_tx_a")).toBe(true);
		expect(await tableExists(testDb.connectionString, "partial_tx_b")).toBe(true);
	});
});
