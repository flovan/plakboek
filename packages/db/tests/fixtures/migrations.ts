...
export function createPartialSplitMigration(): Migration {
	return Object.freeze({
		...
		sql: PARTIAL_FAILURE_STATEMENTS("partial_a", "partial_a_id_idx", "partial_b").join(
...
export function createPartialTransactionalMigration(): Migration {
	return Object.freeze({
		...
		sql: PARTIAL_FAILURE_STATEMENTS("partial_tx_a", "partial_tx_a_id_idx", "partial_tx_b").join(
			"\n",
		),
		transactional: true,
	});
